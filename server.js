// ============================================================
// BTC SNIPER BOT ΓÇö Pure Sniper Engine (No AI)
// Deterministic rule-based trading signals
// Polls exchange klines ΓåÆ computes indicators ΓåÆ sniper engine
// ============================================================
...
// ΓÜá∩╕Å  SSL/TLS cert validation workaround (for dev/testing with expired certs)
// Γ¥ù NEVER use in production ΓÇö this disables critical security
if (process.env.NODE_TLS_REJECT_UNAUTHORIZED === undefined) {
  process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
  console.log("ΓÜá∩╕Å  TLS cert validation DISABLED (dev mode)");
}
...
// ΓöÇΓöÇ Config ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// DATA_SOURCE: bybit | okx | binance | bingx | auto (tries each in order)
const DATA_SOURCE = "bingx"; // ≡ƒöÑ LOCK TO BINGX
...
const POLL_MS = 30000; // 30s ΓÇö safe rate limit (avoid 429)
const KLINE_LIMIT = 250;                                      // enough for EMA200

// ΓöÇΓöÇ Fixed Risk Execution Model ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const FIXED_MARGIN = 5;                                       // USD per trade
const LEVERAGE = 150;                                         // x leverage
const TP_MULTIPLIER = 1.0;                                    // 100% profit on margin
const DRY_RUN = (process.env.DRY_RUN || "false").toLowerCase() === "true";

// ΓöÇΓöÇ Signal Generation Mode ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const MODE = (process.env.MODE || "BALANCED").toUpperCase();  // AGGRESSIVE | BALANCED | SAFE

// ΓöÇΓöÇ Entry Mode: SAFE=closed candle only, AGGRESSIVE=realtime ΓöÇ
const ENTRY_MODE = "AGGRESSIVE"; // AGGRESSIVE = realtime entry, no wait for candle close

// ΓöÇΓöÇ Force Entry Mode ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Per-Pair State ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const pairState = {};
...
// ΓöÇΓöÇ LAB Sniper Engine ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
var labSniper;
...
// ΓöÇΓöÇ Daily Trade Limit ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ WebSocket broadcast ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Bot Logger ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
wss.on("connection", (ws) => {
  console.log("≡ƒôí Dashboard connected");
  botLog("info", "≡ƒôí Dashboard connected");
  ws.isAlive = true;
...
// ΓöÇΓöÇ HTTP helper with timeout + friendly error messages ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function httpGet(url) {
  ...
  try {
    ...
    throw new Error(`network ΓåÆ ${code ? code + " " : ""}${msg}`);
  } finally {
    clearTimeout(t);
  }
}

// ΓöÇΓöÇ BingX perpetual futures klines ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Binance USDT-M futures klines ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function fetchKlinesBinance(symbol, interval, limit) {
  const binSymbol = symbol.replace("-", "");                  // BTC-USDT ΓåÆ BTCUSDT
...
// ΓöÇΓöÇ Bybit linear perpetual klines ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Docs: https://bybit-exchange.github.io/docs/v5/market/kline
async function fetchKlinesBybit(symbol, interval, limit) {
  const bySymbol = symbol.replace("-", "");                   // BTC-USDT ΓåÆ BTCUSDT
...
// ΓöÇΓöÇ OKX perpetual swap klines ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function fetchKlinesOKX(symbol, interval, limit) {
  const okxSymbol = `${symbol}-SWAP`;                         // BTC-USDT ΓåÆ BTC-USDT-SWAP
...
// PURE SNIPER ENGINE ΓÇö No AI, Deterministic Rules
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ

// ΓöÇΓöÇ Session Filter ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function isTradingSession(utcHour) {
  return utcHour >= 0 && utcHour < 24; // Always on (24/7 market)
}

// ΓöÇΓöÇ Market Structure Detection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Real-Time Trigger Detection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getRealtimeTrigger(candle) {
  ...
  // ≡ƒö┤ SHORT TRIGGER: Upper wick > 30% (lowered from 0.4)
  if (upperPct > 0.3) return "SHORT";

  // ≡ƒƒó LONG TRIGGER: Lower wick > 30% (lowered from 0.4)
  if (lowerPct > 0.3) return "LONG";

  return null;
}

// ΓöÇΓöÇ Candle Trigger Detection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getCandleTrigger(candle) {
  return getRealtimeTrigger(candle);
}

// ΓöÇΓöÇ Candle Detail Analyzer ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Entry Confirmation Engine ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Check if price is in zone ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function isPriceInZone(price, zone) {
  return price >= zone[0] && price <= zone[1];
}

// ΓöÇΓöÇ Sniper Recommendation Engine (with Hybrid Fallback Logic) ΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Scalper Recommendation (Fast, Small TP) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getScalperRecommendation(payload) {
  ...
  // ≡ƒƒó LONG: price near/below EMA, RSI oversold
  ...
  // ≡ƒö┤ SHORT: price near/above EMA, RSI overbought
  ...
  // ΓÜí FALLBACK: always return LONG or SHORT (never NONE)
  else if (price < ema20) {
    direction = "LONG";
    entry = price;
...
// ΓöÇΓöÇ Session Detector ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getSession() {
...
// ΓöÇΓöÇ Trend Rider Signal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getTrendRiderSignal(payload, htfBias) {
  ...
  if (emaUptrend && bullStructure && (htfBias === "LONG" || htfBias === "NEUTRAL")) {
    ...
    if (price <= ema20 * 1.005 && price >= ema20 * 0.995) {
      ...
      tp    = price + range * 0.6;
      sl    = ema50 * 0.998;
      reason = `EMA uptrend (20>50>200) + ${structure} structure ΓÇö pullback to EMA20`;
      ...
      tp    = ema20 + range * 0.6;
      sl    = ema50 * 0.998;
      reason = `EMA uptrend + ${structure} ΓÇö wait pullback to EMA20`;
      strength = "waiting";
    }
    ...
    if (price >= ema20 * 0.995 && price <= ema20 * 1.005) {
      ...
      tp    = price - range * 0.6;
      sl    = ema50 * 1.002;
      reason = `EMA downtrend (20<50<200) + ${structure} structure ΓÇö rejection at EMA20`;
      ...
      tp    = ema20 - range * 0.6;
      sl    = ema50 * 1.002;
      reason = `EMA downtrend + ${structure} ΓÇö wait rejection at EMA20`;
      strength = "waiting";
    }
  } else {
    ...
    reason = `no EMA alignment for trend ΓÇö waiting for setup (plan)`;
    strength = "waiting";
  }
...
// ΓöÇΓöÇ Momentum Break Signal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getMomentumBreakSignal(payload) {
  ...
  if (breakResist && strongBody) {
    ...
    reason     = `resistance breakout at ${round(resistance, 2)} ΓÇö strong body ${(bodyPct * 100).toFixed(0)}%`;
    ...
    entry_type = "instant";
    strength   = strongBody && lowWick ? "strong" : "moderate";
    reason     = `support breakdown at ${round(support, 2)} ΓÇö strong body ${(bodyPct * 100).toFixed(0)}%`;
  } else if (rsiVal > 70) {
    direction = "SHORT";
    entry = resistance * 1.001;
    tp    = resistance + range * 0.7;
    sl    = resistance * 0.997;
    reason = `RSI overbought (${rsiVal}) ΓÇö potential breakdown (plan)`;
    entry_type = "waiting";
    strength = "waiting";
  } else if (rsiVal < 30) {
    direction = "LONG";
    ...
    tp    = support - range * 0.7;
    sl    = support * 1.003;
    reason = `RSI oversold (${rsiVal}) ΓÇö potential breakout (plan)`;
    entry_type = "waiting";
    strength = "waiting";
  } else {
    ...
    entry_type = "waiting";
    strength = "waiting";
    reason = `no valid breakout detected ΓÇö waiting breakout setup (plan)`;
  }
...
// ΓöÇΓöÇ Liquidity Sweep Signal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getLiquiditySweepSignal(payload) {
  ...
  if (upperSweep) {
    ...
    reason = `liquidity grab above resistance (${round(resistance, 2)}) ΓÇö upper wick ${(upperWickPct * 100).toFixed(0)}% ΓÇö reversal short`;
  } else if (lowerSweep) {
    direction      = "LONG";
    ...
    reason = `liquidity grab below support (${round(support, 2)}) ΓÇö lower wick ${(lowerWickPct * 100).toFixed(0)}% ΓÇö reversal long`;
  } else {
    const inferredDir = payload.close > (support + resistance) / 2 ? "SHORT" : "LONG";
    direction = inferredDir;
    entry = payload.close;
    ...
    reason = `no sweep yet ΓÇö waiting liquidity sweep (plan)`;
    liquidity_zone = null;
  }
...
// ΓöÇΓöÇ Structure Flip Signal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getStructureFlipSignal(payload, prevStructure) {
  ...
  if (bullFlip) {
    direction    = "LONG";
    reversal_type = `${prevStructure} ΓåÆ ${structure}`;
    entry        = price;
    tp           = price + range * 0.65;
    sl           = support * 0.997;
    ...
    reversal_type = "none";
    confidence   = "low";
    reason = `no flip yet ΓÇö current structure ${structure} (plan)`;
  }
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// PATTERN DETECTION ENGINE ΓÇö Pro Version with Pivot Detection
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓöÇΓöÇ Signal Score (0ΓÇô5 confluence) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Elite Setup (High Confluence Filter) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getEliteSetupSignal(signals, payload, htfBias, momFilter) {
  ...
  if (candidates.length === 0) {
    return {
      ...
      score: 0,
      reason: "no elite setup ΓÇö confluence below threshold",
    };
  }
  ...
  return {
    ...
    rr:    round(best.rr, 2),
    score: best.score,
    reason: `Elite setup from ${best.name} ΓÇö score ${best.score}/5 | ${best.reason}`,
  };
}

// ΓöÇΓöÇ Adaptive Thresholds Per Signal Type ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Select Single Best Signal (adaptive, fallback-enabled) ΓöÇΓöÇΓöÇΓöÇΓöÇ
function selectBestSignal(signals, elite_setup) {
  const candidates = [];

  // Elite setup ΓÇö highest tier
  ...
  if (candidates.length === 0) {
    ...
    return {
      ...
      status:      "CONFIRMED",
      entry_timing: "next_candle",
      reason:      "no valid setup ΓÇö all signals below threshold",
    };
  }

  // Sort: highest score ΓåÆ highest RR ΓåÆ lowest priority
  candidates.sort((a, b) =>
    (b.score - a.score) ||
    ((b.rr || 0) - (a.rr || 0)) ||
...
// ΓöÇΓöÇ Build Full Multi-Strategy Signals ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function buildMultiSignals(payload, htfBias, sniperSignal, prevStructure, patternSignal) {
  ...
  const precision_entry = {
    ...
    reason: isPlan
      ? `${sniperSignal.reason} ΓÇö waiting sniper zone`
      : sniperSignal.reason,
...
// ΓöÇΓöÇ Priority Recommendation (Sniper vs Scalper) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function getPriorityRecommendation(signal) {
  ...
  // ≡ƒöÑ 1. SNIPER only if LONG/SHORT + ACTIVE/READY (NOT SCALP mode)
  if (
    sniper &&
  ...
  // ΓÜí 2. SCALPER (always has direction now, never NONE)
...
// ΓöÇΓöÇ HTF Bias (1H trend confirmation) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Get filter thresholds based on mode ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Core Sniper Signal Generator ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function generateSniperSignal(payload) {
  ...
  // 3. Min range filter (avoid noise scalping) ΓÇö lowered from 0.003 to 0.002
...
// ΓöÇΓöÇ Validate Signal (RR, entry, direction) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function validateSignal(signal) {
  return signal; // ≡ƒöÑ BYPASS ALL VALIDATION FOR DEBUG
}

// ΓöÇΓöÇ Cooldown System ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
let lastTradeTime = 0;
...
// ΓöÇΓöÇ BingX Positions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ BingX Place Order ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Dry Run Simulator ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function simulateDryTrade(signal, payload) {
  ...
  if (!entryZone || !sl) {
    console.log("ΓÜá∩╕Å  Dry run: missing entry zone/sl");
    return;
  }
  ...
  botLog("ok", `≡ƒÄ» DRY TRADE OPENED | ${side} @ ${round(entry, 4)} | TP: ${round(tp, 4)} | SL: ${round(sl, 4)} | Pos: $${positionValue} | RR: ${rr.toFixed(2)}`);

  broadcast({ type: "dry_trade", data: trade });
  broadcast({ type: "dry_trades", data: dryTrades });
}

// ΓöÇΓöÇ Auto-check dry trades against current price ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function checkDryTrades(currentPrice) {
  ...
  dryTrades.forEach(trade => {
    ...
    if (hitTP) {
      botLog("ok", `Γ£à TP HIT | ${side} @ ${currentPrice} | Profit: +$${trade.pnlIfWin}`);
      toClose.push({ tradeId: trade.id, result: "WIN" });
    } else if (hitSL) {
      botLog("warn", `≡ƒ¢æ SL HIT | ${side} @ ${currentPrice} | Loss: -$${trade.pnlIfLose}`);
...
// ΓöÇΓöÇ BingX positions state ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
let latestPositions = null;

// ΓöÇΓöÇ Dispatch: fixed source, or auto-fallback through the list ΓöÇ
async function fetchKlines(symbol, interval, limit = KLINE_LIMIT) {
  ...
  for (const src of order) {
    try {
      ...
      if (activeSource !== src) {
        console.log(`Γ£¿ Data source ΓåÆ ${src.toUpperCase()}`);
        activeSource = src;
      }
      return data;
    } catch (e) {
...
// ΓöÇΓöÇ SMC Overlay Builder (BOS, sweep, trap) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Indicators (EMA, RSI, structure, S/R) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Build market payload for a specific symbol ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function buildMarketPayloadForSymbol(candles, symbol) {
  ...
  return {
    ...
    lastCandle: lastClosed,
    prevCandle: prevClosed,
    candles:    candles, // ≡ƒöÑ FOR CHART
  };
}

// ΓöÇΓöÇ Process a single trading pair ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function processPair(symbol) {
  try {
    ...
    if (candles1m.length === 0) {
      botLog("err", "Γ¥î API returned 0 klines for " + symbol);
      return;
    }
    ...
    // Session filter ΓÇö skip if market is dead
    const utcHour = new Date().getUTCHours();
    if (!isTradingSession(utcHour)) {
      return;
    }

    // Entry lock ΓÇö max 1 entry per minute per pair
    ...
    if (payload1m.barTime !== state.lastAnalyzedBarTime) {
      ...
      botLog("info", `≡ƒôè ${symbol} new bar ΓÇö @ ${payload1m.close} | Structure: ${payload1m.structure}`);
    }
    ...
    // ΓöÇΓöÇ SNIPER (Realtime) Detection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const sniperData = {
      ...signal,
      type: "SNIPER",
      timestamp: Date.now(),
    ...
    // ΓöÇΓöÇ CONFIRMED Detection (when candle closes) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const currentBarTime = payload1m.barTime;
    if (currentBarTime !== state.lastClosedCandleTime) {
      // ≡ƒöÑ candle baru ΓåÆ candle sebelumnya sudah close
    ...
    // ΓöÇΓöÇ PATTERN ENGINE ΓÇö Standard pattern detection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const patternResult = runPatternEngine(candles1m, payload1m);
    signal.pattern_signal = patternResult;
    signal.scalper = scalper;

    // ΓöÇΓöÇ BUILD FINAL DECISION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ SNIPER FUSION (engulfing + OB + FVG + SMC aggregator) ΓöÇΓöÇΓöÇ
    ...
    try {
      ...
      if (sniperFusion) {
        console.log("≡ƒÆÑ SNIPER FUSION:", sniperFusion.type, "| score:", sniperFusion.score, "| reasons:", sniperFusion.reasons);
      }
    } catch(e) {
    ...
    // Apply confirmation candle filter (SAFE mode ΓÇö requires candle close)
    const confirmedDecision = confirmEntry(decision, payload1m);

    // ΓöÇΓöÇ Get priority recommendation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ FORMAT FINAL DECISION SIGNAL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ≡ƒöÑ PATTERN OVERRIDE: If pattern engine finds ENTRY, use pattern as final decision
    let apexDecision;
    if (patternResult.status === "ENTRY") {
      ...
      botLog("ok", `≡ƒÄ» PATTERN SIGNAL OVERRIDE: ${patternResult.pattern} ${patternResult.direction} @ ${patternResult.entry}`);
    } else {
      apexDecision = {
    ...
    state.latestSignal = multiSignals; // ≡ƒöÑ USE multiSignals for UI grid
...
// Broadcast dual signal (sniper + confirmed + pattern)
    ...
    // ΓöÇΓöÇ Broadcast chart_pack for sniper-view ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    var chartPack = {
    ...
    botLog("err", `Γ¥î ${symbol} error: ${err.message}`);
  }
}

// ΓöÇΓöÇ LABUSDT SNIPER ENGINE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function processLabPair() {
  try {
    var labSymbol = "LAB-USDT";
    console.log("≡ƒöÑ LAB SNIPER: Processing...");
...
// ΓöÇΓöÇ Auto PLAN ΓåÆ ACTIVE realtime update ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    return {
      ...
      multi: {
        ...
        best_signal: {
          ...
          status: "ACTIVE",
          reason: signal.multi.best_signal.reason + " ΓÇö entry zone reached"
        }
      }
    };
  }
...
// ΓöÇΓöÇ Main tick: poll exchange, run sniper engine for all pairs ΓöÇ
async function tick() {
  try {
    ...
    // ΓöÇΓöÇ LABUSDT SNIPER ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (labSniper) {
    ...
    try {
      ...
      console.warn("ΓÜá∩╕Å  Positions fetch failed:", e.message);
    }
  } catch (err) {
    console.error(`\nΓ¥î TICK ERROR - Full details:`);
    console.error(err);
    botLog("err", `Fetch failed: ${err.message}`);
  }
}

// ΓöÇΓöÇ POST /simulate ΓÇö instant market data update for testing ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ POST /refresh ΓÇö force an immediate poll & analysis ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ GET /signal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
app.get("/signal", (req, res) => {
  res.json({ signal: latestSignal, market_data: latestPayload });
});

// ΓöÇΓöÇ GET /history ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
app.get("/history", (req, res) => {
  res.json(signalHistory);
});

// ΓöÇΓöÇ GET /trades ΓÇö dry run + live trade history ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ POST /execute-trade ΓÇö BingX auto/manual trade ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
app.post("/execute-trade", async (req, res) => {
  try {
    ...
    if (lastTradeTime && (now - lastTradeTime) < 60000) {
      return res.status(429).json({ error: "cooldown active ΓÇö 60s between trades" });
    }
    ...
    if (DRY_RUN) {
      ...
      botLog("ok", `≡ƒôï DRY TRADE | ${direction} @ ${round(entry, 4)} | TP: ${round(tpPrice, 4)} | SL: ${round(slPrice, 4)}`);
    ...
    botLog("ok", `≡ƒÜÇ LIVE TRADE EXECUTED | ${direction} @ ${round(entry, 4)} | TP: ${round(tpPrice, 4)} | SL: ${round(slPrice, 4)}`);
    ...
    botLog("err", `Γ¥î Trade execution failed: ${err.message}`);
    res.status(500).json({ error: err.message });
  }
});

// ΓöÇΓöÇ GET /diag ΓÇö connectivity test to all exchanges ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ GET /candles ΓÇö chart data for frontend ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ GET /health ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ GET /positions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ POST /dryrun ΓÇö toggle dry run mode ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
app.post("/dryrun", (req, res) => {
  const { action, tradeId, result } = req.body;
  if (action === "toggle") {
    dryRunActive = !dryRunActive;
    console.log(`≡ƒÄ« Dry run ${dryRunActive ? "ACTIVATED" : "DEACTIVATED"}`);
...
// ΓöÇΓöÇ Start server & begin polling ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const PORT = process.env.PORT || 3001;
server.listen(PORT, async () => {
  botLog("ok", `≡ƒÜÇ BTC SNIPER BOT STARTED | Mode: ${DRY_RUN ? "≡ƒº¬ DRY RUN" : "≡ƒÜÇ LIVE"}`);
  botLog("info", `ΓÜÖ∩╕Å Config: Margin=$${FIXED_MARGIN} | Leverage=${LEVERAGE}x | Source=${DATA_SOURCE}`);
  botLog("info", `≡ƒôè Timeframe: ${INTERVAL} | Poll interval: ${POLL_MS}ms`);
  console.log(`
...
Γòæ       BTC SNIPER BOT ΓÇö Fixed Risk Execution Engine     Γòæ
ΓòáΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòú
Γòæ  Mode      ΓåÆ ${(DRY_RUN ? "≡ƒº¬ DRY RUN" : "≡ƒÜÇ LIVE TRADING").padEnd(40)}Γòæ
Γòæ  Dashboard ΓåÆ http://localhost:${PORT}                     Γòæ
Γòæ  WS        ΓåÆ ws://localhost:${PORT}                       Γòæ
Γòæ  Source    ΓåÆ ${(DATA_SOURCE === "auto" ? "AUTO (bybitΓåÆokxΓåÆbinanceΓåÆbingx)" : DATA_SOURCE.toUpperCase()).padEnd(36)}Γòæ
Γòæ  Symbols   ΓåÆ ${SYMBOLS.join(",").padEnd(36)}Γòæ
Γòæ  Timeframe ΓåÆ ${INTERVAL.padEnd(36)}Γòæ
Γòæ  Poll      ΓåÆ every ${(POLL_MS / 1000).toString().padEnd(26)}s   Γòæ
Γòæ                                                        Γòæ
Γòæ  Margin    ΓåÆ $${FIXED_MARGIN} | Leverage ΓåÆ ${LEVERAGE}x | Profit Target ΓåÆ ${(TP_MULTIPLIER * 100).toFixed(0)}%Γòæ
Γòæ  Risk ModelΓåÆ Fixed $${FIXED_MARGIN} margin per trade            Γòæ
Γòæ                                                        Γòæ
Γòæ  /signal   ΓåÆ GET   latest signal                       Γòæ
Γòæ  /history  ΓåÆ GET   last 50 signals                     Γòæ
Γòæ  /health   ΓåÆ GET   server status                       Γòæ
Γòæ  /refresh  ΓåÆ POST  force re-analyze now                Γòæ
Γòæ  /simulate ΓåÆ POST  mock data (dev)                     Γòæ
...
`);
  // Don't await ΓÇö start polling in background so server can listen immediately
  tick().catch(e => console.error("Initial tick error:", e));
  setInterval(tick, POLL_MS);

  // Realtime price update + PLANΓåÆACTIVE status change (every poll, not just candle close)
  setInterval(async () => {
    try {
  ...
  // ΓöÇΓöÇ Realtime WebStream (Binance 1m kline) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  let ltfCandles = [];
  ...
  startStream({
    ...
    onConnect: function() { console.log("ΓÜí Realtime stream connected"); },
    onDisconnect: function() { console.log("ΓÜí Realtime stream disconnected"); }
  });
